---
description: Restore Claude's memory across the 5 workspace repos — load each repo's MEMORY.md index, then pull detail files on relevance
---
...
different repo. This command pulls it back into context across all 5 repos at once.

If an argument is given (`$ARGUMENTS`), restore only the repo whose name it matches; otherwise restore all 5.

**Ignore `sp500_index_bot`** — that strategy is declared sub-optimal for trading and is intentionally out of scope.

## The 5 repos and their memory dirs
...
3. After loading, give a **concise per-repo summary**: one or two lines each, led by a bottom line:
   **"Restored memory for <N>/5 repos."** Flag any repo with no memory yet.
